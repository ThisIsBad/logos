...
LLMs verlieren nach jeder Session ihren gesamten Kontext. MemoryBrain speichert
Erkenntnisse, Fakten und Ereignisse dauerhaft — und macht sie semantisch abrufbar.
Memories die über LogicBrain verifiziert wurden, erhalten das `proven=True` Flag.
...
| `list_proven_beliefs` | Gibt alle Memories zurück die via LogicBrain verifiziert sind |
...
└── LogicBrain-Client        → Optionale Verifikation beim Speichern
```
...
- SQLite (Metadaten)
- LogicBrain-Client (optional, für `proven` Flag)

## Deployment (Railway)

Folgt dem LogicBrain-Muster: `Dockerfile` + `Procfile` + `railway.toml`
