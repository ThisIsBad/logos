...
| **2** | Tool Agent | ✅ Erreicht | MCP-Tools, Logos deployed |
...
|-----|----------------------------------|------------------------|
| Verifikation | Plausibel ≠ korrekt, kein Falsifikations-Mechanismus | **Logos** ✅ |
| Ziel-Governance | Ohne explizite Ziel-Repräsentation: Drift | **Logos** (GoalContract) + **Telos** |
| Persistentes Gedächtnis | Context Windows sind ephemer, kein lifelong learning | **Mneme** |
| Planning & Search | Autoregressive Planung, kein State-Space-Search | **Praxis** |
| Selbst-Modellierung | Kalibrierung ist ungelöstes Problem, aber strukturell lösbar | **Episteme** |
| Welt-Modellierung | Korrelationen ≠ Kausalstruktur, kein Do-Calculus | **Kosmos** |
| Aktives Lernen | Kein Exploration-Exploitation-Loop nach Deployment | **Empeiria** |
| Skill-Akkumulation | Strategien sind nicht persistent speicherbar/abrufbar | **Techne** |
...
- Verifizierte Erinnerungen: Memories mit Logos-ProofCertificate → `proven=True`
...
#### `Praxis` — Hierarchisches Planning & Search
...
- Plan-Verifikation via Logos (GoalContract pre/postconditions)
...
verify_plan(plan_id) → Logos GoalContract check
commit_step(plan_id, step_id, outcome)
get_plan(plan_id)
```

**Stack:** Python 3.12 + NetworkX (Graphstruktur) + Logos-Client + FastAPI + MCP  
**Status:** 🔲 Nicht implementiert
...
#### `Kosmos` — Kausales Reasoning (Do-Calculus)
...
- Integration mit Logos `counterfactual_branch` für formale Absicherung
...
**Stack:** Python 3.12 + NetworkX + pgmpy + Logos-Client + FastAPI + MCP  
**Status:** 🔲 Nicht implementiert
...
#### `Empeiria` — Erfahrungs-Akkumulation
...
- Kandidaten für Techne-Promotion identifizieren
...
- Nur Skills mit Logos-ProofCertificate → `verified=True`
- Semantische Suche: "Finde Skill für Task X"
- Promotion-Pipeline: Empeiria → Techne
...
**Stack:** Python 3.12 + ChromaDB + SQLite + Logos-Client + FastAPI + MCP  
**Status:** 🔲 Nicht implementiert

---

#### `Telos` — Ziel-Stabilitäts-Monitor

**AGI-Lücke:** Goal stability, mesa-optimization prevention  
**Inspiration:** Omohundro (2008), Hubinger et al. (2019)

**Architektur:**
- Registrierte Ziele mit Constraints (via Logos GoalContract)
...
**Stack:** Python 3.12 + SQLite + Logos-Client + FastAPI + MCP  
**Status:** 🔲 Nicht implementiert
...
- Logos als optionaler Verifikations-Backend für alle Services
- Kein direktes Service-zu-Service-Calling (Claude orchestriert)
```

Jeder Service folgt dem Logos-Deployment-Muster:
```
<service>/
...
|------|---------|-------|---------|--------|
| **1** | `Mneme` | Persistenz | Mittel | Sehr hoch |
| **2** | `Praxis` | Planning | Mittel-Hoch | Hoch |
| **3** | `Episteme` | Kalibrierung | Niedrig | Hoch |
| **4** | `Kosmos` | Kausalität | Hoch | Hoch |
| **5** | `Empeiria` | Lernen | Niedrig | Mittel (Stage 4) |
| **6** | `Techne` | Skills | Mittel | Mittel (Stage 4) |
| **7** | `Telos` | Drift | Niedrig | Mittel (Logos-Extension) |
