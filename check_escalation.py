"""Check escalation round answers."""
import json, sys

round_name = sys.argv[1] if len(sys.argv) > 1 else "round1"

with open(f"results/escalation_{round_name}.json", encoding="utf-8") as f:
    exam = json.load(f)
with open(f"results/escalation_{round_name}_answers.json", encoding="utf-8") as f:
    llm = json.load(f)
