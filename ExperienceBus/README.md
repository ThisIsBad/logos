...
Ohne ExperienceBus beginnt jeder Task bei Null. Mit ExperienceBus akkumuliert
...
| `get_skill_candidates` | Erfahrungen die reif für SkillLibrary-Promotion sind |
...
```
ExperienceBus.record_experience()
    → extract_patterns() identifiziert bewährte Strategie
    → get_skill_candidates() schlägt Promotion vor
    → SkillLibrary.promote_experience() speichert als Skill
    → (optional) LogicBrain.certify_claim() verifiziert den Skill
```
